...
{
    public class ActorModelUIPart : BaseModelUIPart
    {
        ...
        public static ActorModelUIPart Create(string name, IGraphElementModel model, IModelUI ownerElement, string parentClassName)
        {
            if (model is INodeModel)
            {
                return new ActorModelUIPart(name, model, ownerElement, parentClassName);
            }

            return null;
        }
        protected ActorModelUIPart(string name, IGraphElementModel model, IModelUI ownerElement, string parentClassName)
        ...
        {
            DialogueActorModel actor = m_Model as DialogueActorModel;
            m_DialogueStringField.SetValueWithoutNotify(actor.Name);
        }
    }
}
